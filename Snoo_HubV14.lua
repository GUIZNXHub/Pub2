...
SmoothTexturesEnabled = LoadedConfig.SmoothTexturesEnabled == true
...
-- ==================== SMOOTH TEXTURES / MATERIAL OPTIMIZER ====================
-- Troca somente o Material dos BaseParts para SmoothPlastic.
-- O Material original de cada peça é guardado em memória e restaurado ao desligar.
-- Texturas/Decals/Meshes não são destruídos nem alterados.
local function SmoothTexturesSetEnabled(State)
	State = State == true

	if State == SmoothTexturesEnabled then
		return
	end
	...
	if State then
		...
		-- Uma única varredura ao ativar: não fica rodando em loop.
		for _, Object in ipairs(workspace:GetDescendants()) do
			if Object:IsA("BasePart") then
		...
		-- Peças novas também ficam SmoothPlastic enquanto o modo estiver ativo.
		...
		-- Restaura exatamente o Material que cada peça tinha antes.
...
local SmoothTexturesHint = Description("Deixa os materiais lisos para reduzir o peso visual. Ao desligar, restaura os materiais originais.")
...
if SmoothTexturesEnabled then
	-- O modo salvo é reativado somente depois que a UI e as funções existem.
	SmoothTexturesEnabled = false
...
print("SnooHubV14 carregado corretamente • Texturas Smooth disponível em Config.")
