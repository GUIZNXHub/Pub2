...
local function ESPFindBestTarget()
	local Plots = workspace:FindFirstChild("Plots")
	...
	-- Procura TODOS os modelos numerados de 1 a 8 dentro de Plots.
	for _, Object in ipairs(Plots:GetDescendants()) do
		local IsSlot, SlotNumber = ESPIsSlot(Object)

		if IsSlot and not SeenSlots[Object] then
			SeenSlots[Object] = true

			-- O nome do Brainrot pode estar no próprio modelo ou em qualquer
			-- descendente desse modelo.
			local function CheckCandidate(Candidate)
				...
				if Name and Priority and Part then
...
local function ESPUpdate()
	...
	if not Target
		...
		ESPCurrentTarget = ESPFindBestTarget()
		Target = ESPCurrentTarget
	end
	...
	local TargetPart = ESPGetPart(Target.Object)
