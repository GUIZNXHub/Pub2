--========================================================
-- SNOO HUB X2
-- USAR E IA E PARA BURRO ALRENDEPRENDE FAZER SCRIPT😭✌️
--========================================================
...
Section("CÂMERA")
Label("FOV")
local FOVBox = TextBox("70", "Digite o FOV")
local FOVButton = Button("  FOV • APLICAR")
...
AutoStealEnabled = LoadedConfig.AutoStealEnabled == true
StealDuration = ApplyLoadedNumber("StealDuration", 1.4, 0.1, 10)
StealRadius = ApplyLoadedNumber("StealRadius", 60, 1, 500)
ClearTable(PriorityList)
if type(LoadedConfig.PriorityList) == "table" and #LoadedConfig.PriorityList > 0 then
	for _, name in ipairs(LoadedConfig.PriorityList) do
		if type(name) == "string" and name ~= "" then table.insert(PriorityList, name) end
	end
end
if #PriorityList == 0 then
	for _, name in ipairs(DefaultPriorityList) do table.insert(PriorityList, name) end
end
...
local FOVValue = Clamp(tonumber(LoadedConfig.FOVValue) or OriginalFOV, 1, 120)
FOVBox.Text = tostring(math.floor(FOVValue + 0.5))

local function ApplyFOV(Value)
	local Cam = workspace.CurrentCamera
	if not Cam then return end
	FOVValue = Clamp(Value, 1, 120)
...
FOVButton.MouseButton1Click:Connect(function()
	...
	if Number then ApplyFOV(Number) end
...
SaveConfig = function()
	...
	local Data = {
		WalkValue = WalkValue, JumpValue = JumpValue, TravelValue = TravelValue, TPValue = TPValue,
		...
		UltraESPEnabled = UltraESPEnabled, Language = Language, FOVValue = FOVValue,
