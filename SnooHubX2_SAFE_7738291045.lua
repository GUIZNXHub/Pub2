...
-- SNOO HUB X2 SAFE
-- Código: 7738291045
-- UI e funções idênticas + proteção total contra nil
--========================================================

print("[SnooHubX2] Iniciando...")
...
if not Player then
	warn("[SnooHubX2] LocalPlayer não encontrado. Use LocalScript + Play ou executor.")
	return
end
...
if not PlayerGui then
	...
	if ok then PlayerGui = gui end
end
if not PlayerGui then
	warn("[SnooHubX2] PlayerGui não encontrado.")
	return
end

print("[SnooHubX2] Player OK")

-- Wrappers seguros (não existem no Studio / cliente oficial)
local function SafeIsFile(name)
	local ok, res = pcall(function()
		return type(isfile) == "function" and isfile(name)
	end)
	return ok and res
end
...
local function SafeReadFile(name)
	local ok, res = pcall(function()
		if type(readfile) == "function" then return readfile(name) end
		return nil
	end)
...
pcall(function()
	if SafeIsFile(ConfigFileName) then
		local content = SafeReadFile(ConfigFileName)
		if type(content) == "string" and #content > 2 then
...
local function ApplyLoadedNumber(Key, Default, Min, Max)
	...
	if Value then return Clamp(Value, Min, Max) end
	return Default
end
...
-- Carrega do config ou usa o padrão
...
local function RebuildPriorityMap()
	-- só para uso interno se precisar
end

local function SetupCharacter(CharacterObject)
	if not CharacterObject then return end
	Character = CharacterObject
	local okH, h = pcall(function() return Character:WaitForChild("Humanoid", 5) end)
	local okR, r = pcall(function() return Character:WaitForChild("HumanoidRootPart", 5) end)
	if okH then Humanoid = h end
	if okR then Root = r end
	if Humanoid and WalkEnabled then
		pcall(function() Humanoid.WalkSpeed = WalkValue end)
	end
...
if Player.Character then
	pcall(SetupCharacter, Player.Character)
end
...
Player.CharacterAdded:Connect(function(CharacterObject)
	pcall(SetupCharacter, CharacterObject)
end)
...
DiscordButton.MouseButton1Click:Connect(function()
	SafeSetClipboard("https://discord.gg/v2dwtQAqan")
	DiscordButton.Text = "  DISCORD COPIADO"
	task.delay(1.5, function()
		if DiscordButton and DiscordButton.Parent then DiscordButton.Text = "  DISCORD • COPIAR" end
	end)
end)
...
local function ExecuteAutoSteal(Prompt)
	...
	if not StealData[Prompt] then
		StealData[Prompt] = {Hold = {}, Trigger = {}, Ready = true, UseFire = false}
		local connsHold = SafeGetConnections(Prompt.PromptButtonHoldBegan)
		local connsTrig = SafeGetConnections(Prompt.Triggered)
		for _, Connection in ipairs(connsHold) do
			if type(Connection) == "table" and type(Connection.Function) == "function" then
				table.insert(StealData[Prompt].Hold, Connection.Function)
			elseif type(Connection) == "function" then
				table.insert(StealData[Prompt].Hold, Connection)
			end
		end
		for _, Connection in ipairs(connsTrig) do
			if type(Connection) == "table" and type(Connection.Function) == "function" then
				table.insert(StealData[Prompt].Trigger, Connection.Function)
			elseif type(Connection) == "function" then
				table.insert(StealData[Prompt].Trigger, Connection)
			end
		end
		if #StealData[Prompt].Hold == 0 and #StealData[Prompt].Trigger == 0 then
			StealData[Prompt].UseFire = true
		end
	end
	...
	task.spawn(function()
		if Data.UseFire then
			...
			for _, Callback in ipairs(Data.Hold) do
				pcall(function() task.spawn(Callback) end)
			end
			while IsStealing and tick() - StealStartTime < StealDuration do
				local Progress = Clamp((tick() - StealStartTime) / StealDuration, 0, 1)
...
ResetFOVButton.MouseButton1Click:Connect(function()
	if ResetFOV() then
		ResetFOVButton.Text = "  RESET FOV • ORIGINAL"
		if SaveConfig then SaveConfig() end
	end
end)
...
-- ==================== LÓGICA DE MOVIMENTO (resumida para espaço) ====================
SelectPage("Movement")
...
SpeedButton.MouseButton1Click:Connect(function()
	...
	if not WalkEnabled then
		...
		if Number then WalkValue = Clamp(Number, 1, 500) end
...
local function StartTravel()
	...
	if Number then TravelValue = Clamp(Number, 1, 1000) end
	Traveling = true
	CancelTravel = false
	TravelButton.Text = "CANCELAR TRAJETO"
	local Destination = TargetCFrame.Position
	local StartPosition = Root.Position
	local FlightHeight = Clamp(math.max(StartPosition.Y + 25, Destination.Y + 25), -50000, 50000)
...
local function StartStepTeleport()
	...
	if Number then TPValue = Clamp(Number, 1, 1000) end
	StepTeleporting = true
	CancelStepTeleport = false
	TPButton.Text = "CANCELAR TP"
	local Destination = TargetCFrame.Position
	local StartTime = os.clock()
	local MaxDuration = Clamp((Destination - Root.Position).Magnitude / math.max(TPValue, 1) + 10, 10, 180)
...
local BugSpamTPButtonSmall = nil
...
local function StartBugSpamTP()
	...
	BugSpamTPConnection = RunService.Heartbeat:Connect(function()
		...
		task.spawn(function()
			pcall(function()
				-- Teleporta 15 studs à frente da posição salva
				...
				-- Arrasta rapidamente até o ponto salvo com mini subidas e descidas
				...
				while BugSpamTPEnabled and Root and Root.Parent do
					...
					-- Mini subidas e descidas (bobbing)
...
-- FPS / Noclip / Prompt / Jump (mantidos)
local FPSOriginals = {}
...
JumpButton.MouseButton1Click:Connect(function()
	...
	if not JumpEnabled then
		...
		if Number then JumpValue = Clamp(Number, 1, 500) end
		Humanoid.UseJumpPower = true
		Humanoid.JumpPower = JumpValue
		JumpEnabled = true
...
local SpamTPV1ButtonSmall = Instance.new("TextButton", FloatingTP)
...
local SpamTPV2ButtonSmall = Instance.new("TextButton", FloatingTP)
...
SaveConfig = function()
	pcall(function()
		local fx, fy = 20, 300
		if FloatingTP then
			fx = FloatingTP.Position.X.Offset
			fy = FloatingTP.Position.Y.Offset
		end
		local Data = {
		...
		SafeWriteFile(ConfigFileName, HttpService:JSONEncode(Data))
	end)
end
...
UserInputService.InputChanged:Connect(function(Input)
	...
	if Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch then
		...
		local NewX = Clamp(FloatingStartPos.X.Offset + Delta.X, 4, math.max(4, Viewport.X - FloatingTP.AbsoluteSize.X - 4))
		local NewY = Clamp(FloatingStartPos.Y.Offset + Delta.Y, 4, math.max(4, Viewport.Y - FloatingTP.AbsoluteSize.Y - 4))
...
local function AttachBestBeam(targetPart)
	...
	if root then
		...
		local curve = Clamp(heightDiff * 0.09, -14, 20)
...
if InitialCamera and OriginalFOV == nil then OriginalFOV = InitialCamera.FieldOfView end
FOVBox.Text = tostring(math.floor(FOVValue + 0.5))
...
local FinalCamera = GetCamera()
if FinalCamera then
	if OriginalFOV == nil then OriginalFOV = FinalCamera.FieldOfView end
	if FOVValue ~= OriginalFOV then FinalCamera.FieldOfView = FOVValue end
end
SaveConfig()
...
print("[SnooHubX2] SAFE carregado com sucesso! Código: 7738291045")
